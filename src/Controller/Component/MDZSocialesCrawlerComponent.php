...
/**
 * Description of MDZSocialesCrawlerComponent
 *
 * @author Luciano
 */
class MDZSocialesCrawlerComponent extends BaseCrawlerComponent{   
    ...
    public function getDescripcion(){
        ...
        $data1 = $this->html->find('.entry-summary', 0);
    ...
    public function getContenido(){
        ...
        if(!is_null($data2)){
            foreach($data2->children as $data){
                ...
                //twitter timeline
                elseif($data->tag == "figure" && !is_null($data->find('a', 0)) && strpos($data->find('a', 0)->attr['class'], "twitter") !== false && strpos($data->find('a', 0)->attr['class'], "timeline") !== false){
    ...
    public function getImagenes(){
        ...
        if(!is_null($data3)){
            $json = end($data3->find('script'));
            if(!is_null($json)){
                //obtenemos el JSON del tag
                $end = end(explode("vplfgal", $json->innertext));
                $json_no_inicio = "[" . substr($end, 12, strlen($end));
                $json_no_inicio_ni_fin = substr(reset(explode("false", $json_no_inicio)), 0, -8) . "]}]";
                //convertimos a JSON
    ...
    public function getFechaPublicado(){
        ...
        if(!is_null($date_pub)){
            $date_pub = $this->html->find('time', 0)->attr['datetime'];
            $fecha = str_replace("T", " ", substr($this->html->find('time', 0)->attr['datetime'], 0, -6));
        }
