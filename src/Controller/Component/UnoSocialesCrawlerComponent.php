...
/**
 * Description of UnoSocialesCrawlerComponent
 *
 * @author Luciano Buttazzoni
 */
class UnoSocialesCrawlerComponent extends BaseCrawlerComponent{   
                            ...
                            if($this->existTitle(trim(strip_tags($link['titulo']))) && !in_array(trim(strip_tags($link['titulo'])),$guardados[$this->codigo])){
                                try{
                                    ...
                                    if($this->html){
                                        ...
                                        //$articulo->claves = $this->getPalabrasClaves();
        ...
        if(!is_null($data2) && !is_null($data2->children)){
            foreach($data2->children as $data){
                ...
                elseif($data->tag == "div" && !is_null($data->find('iframe', 0)) && strpos($data->find('iframe', 0)->attr['src'], "youtu") !== false){
                    ...
                    $video_you_src_link_tmp = "https:" . end(explode(':', $video_you_src->attr['src']));
    ...
    public function getLinksPortada(){
        ...
        $titular_items = $this->html->find('.simple-list-with-summary', 0);
        if(!is_null($titular_items)){                
            $destacados = $titular_items->find('h1', 0)->find('a', 0);
            $links[] = [
                'titulo' => html_entity_decode($destacados->attr['title']),
                'link' => $destacados->attr['href']
                ];
        }

        $lista_items = $this->html->find('.section-article-list-with-summary', 0);
        if(!is_null($lista_items)){
            foreach($lista_items->children as $article){
                if(isset($article->find('a', 0)->attr['href']) && !is_null($article->find('a', 0)->attr['href']) && isset($article->find('a', 0)->attr['title']) && !is_null($article->find('a', 0)->attr['title'])){
                    $links[] = [
                        'titulo' => html_entity_decode($article->find('a', 0)->attr['title']),
                        'link' => $article->find('a', 0)->attr['href']
                        ];
                }
            }
        }

        /*$portada_items = $this->html->find('.search-results-list', 0);
        if(!is_null($portada_items)){
            unset($portada_items->children[0]);
            unset($portada_items->children[2]);
            foreach($portada_items->children as $article){
                if(!is_null($article->find('a', 0)) && !is_null($article->find('a', 0)->attr['href'])){
                    $links[] = [
                        'titulo' => html_entity_decode($article->find('a', 0)->attr['title']),
                        'link' => $article->find('a', 0)->attr['href']
                        ];
